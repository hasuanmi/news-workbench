...
class XxsbScraper(BaseScraper):
    """信息时报 — 官网 xxsb.com / 数字报 epaper.xxsb.com。"""
    media = "信息时报"
    business = ["daily_review"]
    source_type = "官方网站"
    entry_urls = [
        "http://www.xxsb.com/",
        "http://epaper.xxsb.com/",
    ]
    ...
    async def list_articles(self):
        ...
        for url in self.entry_urls:
            try:
                ...
                logger.warning(f"[xxsb] 列表页失败 {url}: {e}")
                continue
            soup = BeautifulSoup(html, "lxml")
            stubs += collect_links(soup, url, min_cn=8, allowed_hosts=["xxsb.com"])
        seen, uniq = set(), []
        for s in stubs:
