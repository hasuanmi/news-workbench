...
class XkbScraper(BaseScraper):
    """新快报 — 官网 xkb.com.cn / 数字报 epaper.xkb.com.cn。"""
    media = "新快报"
    ...
    async def list_articles(self):
        ...
        for url in self.entry_urls:
            try:
                ...
                logger.warning(f"[xkb] 列表页失败 {url}: {e}")
                continue
            soup = BeautifulSoup(html, "lxml")
            stubs += collect_links(soup, url, min_cn=8, allowed_hosts=["xkb.com.cn"])
        seen, uniq = set(), []
        for s in stubs:
