...
class GzdailyScraper(BaseScraper):
    media = "广州日报"
    ...
    async def list_articles(self):
        ...
        for url in self.entry_urls:
            try:
                ...
                logger.warning(f"[gzdaily] 列表页失败 {url}: {e}")
                continue
            soup = BeautifulSoup(html, "lxml")
            stubs += collect_links(soup, url, min_cn=8, allowed_hosts=["dayoo.com"])
        seen, uniq = set(), []
        for s in stubs:
    ...
    async def parse_detail(self, url, html):
        ...
        h1 = soup.find("h1") or soup.select_one(".article-title, .title, .art_title")
