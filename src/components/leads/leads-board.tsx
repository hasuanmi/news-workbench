...
const CLUE_TYPE_COLORS: Record<string, string> = {
  new_column: "bg-[#b3392f] text-white",
  series: "bg-[#c87f2d] text-white",
  special_topic: "bg-[#6b6257] text-white",
...
export function LeadsBoard() {
  ...
  return (
    <div className="space-y-4">
      {/* 筛选栏 */}
      <div className="flex flex-wrap items-center gap-3 sticky top-0 bg-[#faf7f2] z-10 py-3 border-b border-[#e8e2d8]">
        <Input
          type="date"
          value={dateFilter}
        ...
        <div className="ml-auto text-sm text-[#6b6257]">
          共 {total} 条线索
        </div>
      </div>
            ...
            <Card key={clue.id} className="border-[#e8e2d8] shadow-none">
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  {/* 类型徽章 */}
                  <Badge className={CLUE_TYPE_COLORS[clue.clue_type ?? ""] ?? "bg-[#6b6257] text-white shrink-0"}>
                    ...
                    <div className="flex items-center gap-3 text-xs text-[#6b6257]">
                      ...
                      {clue.confidence > 0 && (
                        <span className="flex items-center gap-1">
                          置信度
                          <span className="inline-block w-12 h-1.5 bg-[#e8e2d8] rounded-full overflow-hidden">
                            <span
                    ...
                    {clue.tags && clue.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {clue.tags.map((tag) => (
                          <span
                            key={tag}
                            className="px-1.5 py-0.5 text-xs bg-[#f5f0e8] text-[#6b6257] rounded"
                          >
                            {tag}
                          </span>
                        ))}
