...
export async function GET(req:NextRequest){
  ...
  return Response.json({connectedMediaCount:3,mediaNames:['广州日报','南方日报','南方都市报'],scrape:{status:scrapeStatus,phase:scrape?.phase??null,startedAt:scrape?.started_at??null,endedAt:scrape?.ended_at??null,articles:mediaSteps.reduce((n:number,s:{articles?:number})=>n+(s.articles??0),0),successfulMedia:mediaSteps.filter((s:{ok?:boolean})=>s.ok).length,error:scrape?.error??null,osResult:triggers.clue_identify?.lastResult??null},pendingArticles24h:error?null:count,pendingError:error?.message??null,identification:identification?{runId:identification.run_id,startedAt:identification.started_at,executed:identification.executed,...identification.summary,processed:identification.processed,errors:identification.errors}:null});
}
