...
> 本目录是 **news-workbench**（AI 新闻辅助工作台）仓库内置的抓取服务，与主系统同属一个项目、一个仓库。
> 主系统（Next.js）负责 AI 业务，本服务（Python）负责**确定性采集**，两者通过 `GET /api/ingest/queue` / `POST /api/ingest/articles` 对接。

独立的「原始新闻数据抓取服务」，用于解决扣子(Coze)云端沙箱无法稳定访问新闻网站的问题。
**只负责确定性数据采集**，不负责「是否重点稿 / 是否新栏目 / 是否同题」等 AI 业务判断（这些留在主项目）。

对外提供 HTTP API，供主项目在需要时拉取原始文章数据；可长期运行于 Linux 云服务器（Docker 部署）。
