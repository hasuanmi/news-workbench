...
| 模块 | 状态 | 说明 |
|------|------|------|
| 新闻日历 | ✅ 已交付 | 全年节点底图、当前年动态 AI 补充、历史回看、跨年固定节点预览 |
...
- 默认展示中国时区当前年度全年节点；列表按日期升序，包含已过日期；月历可跨年翻页。未来30天仅为快捷筛选，首页摘要保留配置的天数。
- 历史导入按原年度直接回看，无需先迁移到正式事件表。固定日期、农历节日、节气和明确周年按目标年推导；会议、政策、活动及假期安排不直接复制到下一年。
- AI 推荐限定当前年度动态事件并保留来源依据。时间待定可以保留具体事件；模糊名称、只有月份的标题及缺少必要信息的记录标为“信息待补全”，不进入正式日历。
- 来源统一为历史迁移、AI推荐、用户新增、用户粘贴识别。跨年推导和历史查询不写回原始数据。
