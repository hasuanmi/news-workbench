...
- **节点自动补全（AI，后台自动、前台仅展示）**：`calendar_event` 新增 `ai_background`/`ai_why`/`ai_topics`/`ai_sources`/`enrich_status`(none|pending|processing|completed|failed)/`enrich_fingerprint`/`enrich_error`/`enrich_fail_count`/`enriched_at`。节点被新增、历史迁移、AI 推荐或用户粘贴识别后，由 `src/lib/calendar-enrich.ts` **自动触发**补全（无需用户点击）：计算信息指纹（名称/日期/地区/分类）→ 联网检索（`coze-coding-dev-sdk` SearchClient，优先权威站点：中国政府网/新华社/人民日报/央视/粤穗政府等）→ 结合检索上下文调大模型生成「背景信息/为什么值得关注/可参考的选题方向」→ 落库。检索无可靠来源不编造，置 `completed`（`ai_sources` 为空，详情展示「暂未检索到可靠来源」）。指纹相同且已 `completed` 跳过（省 Token）；节点名称/日期/地区/分类修改自动重新触发（PATCH 关键字段变化时 force，重置 `enrich_fail_count`）。**失败自动重试**：LLM/落库失败置 `failed` 并递增 `enrich_fail_count`，详情路由 `GET /api/calendar/[id]` 读到 `failed` 且未达上限时自动触发 `maybeAutoRetryEnrich` 重试；前端详情面板无主入口按钮——`pending/processing` 显示「正在自动补全…」、`failed` 显示「自动补全失败，系统将自动重试」，数据自动轮询刷新；仅当失败**超过自动重试上限**（`calendar.enrich.max_retries`，默认 3）时才显示次级「重新生成」异常恢复按钮（调 `POST /api/admin/calendar/[id]/enrich`）。开关/权威站/来源数/重试上限读 `app_config` 的 `calendar.enrich.*`。触发入口在创建 `POST /api/admin/calendar` 与编辑 `PATCH /api/admin/calendar/[id]`（关键字段变化时 force）。
...
- **补充要求 → 重新生成完整评报（WF04-F 重构）**：`followup` 不再是无状态聊天问答，而是「用户补充要求后重新生成一版完整评报」。`src/lib/review-followup.ts` 提供 `regenerateStructure`（结构化四区块 JSON：today_focus/same_topic/peer_highlights/gz_daily）与 `streamRegeneratedFinalReview`（最终评报流式），上下文 = 原始四区块 + final_summary + 对比媒体/维度/条件 + 用户新增要求，约束模型只基于材料、不新增事实、输出严格 JSON（前端组件渲染，不塞 Markdown 原文）。路由 `POST /api/review/[id]/followup`（SSE 事件 start→structure→final→saved→done/error，含客户端断连防护），前端 `src/components/review/review-followup.tsx` 挂载于评报结果区：输入补充要求 → 重新生成 → 全量替换保存为新版本。
- **版本快照与恢复**：`daily_review_revision` 表（review_id/version/sections/final_summary/source=generate|followup|restore/change_note/created_by）。`applyFollowupRegeneration`（review-engine.ts）在每次重新生成前快照当前版本、再全量替换 sections（保留 conditions）与 final_summary、version+1；`GET /api/review/[id]/revisions` 列版本，`POST /api/review/[id]/revisions/restore` 按 revisionId 恢复（恢复前再快照）。前端追问组件支持「历史版本（原版本/当前版本）」面板查看/恢复。
...
- **触发方式**：`POST /api/cron/{job}`（外部 crontab / Vercel Cron / 云函数，用 `Authorization: Bearer <CRON_SECRET>` 鉴权，也接受 admin cookie）；后台「系统管理 → 定时任务状态」（`/admin/scheduler`）可配置开关与 cron 表达式、查看运行状态/版本日志、点「立即执行」手动触发（`POST /api/admin/scheduler/run`，manual 不受开关限制）。
- **自动运行判定（如实告知）**：本系统不会在进程内跑 setInterval；是否真正每日自动执行由「是否配置触发密钥 + 服务器 crontab 定时调用 `/api/cron/{job}`」决定。`GET /api/admin/scheduler` 返回 `autoScheduled`（= 是否配置了 CRON_SECRET/CRON_TOKEN）、每个任务的 `lastRunAt/lastSuccessAt/lastProcessedCount/lastError/nextRunAt`（cron-parser 推算下一次执行）与 `message`，未配置密钥时明确提示「尚未自动运行，仅支持手动立即执行」。当前 DEV/沙箱环境**未配置** CRON_SECRET，线上不会自动执行抓取/线索识别/评报生成，仅可后台手动触发；正式部署后按 `DEPLOY.md`「定时任务」配置 crontab 方可自动运行。
